...
function AdminPanel({ role }: { role: string }) {
  ...
  const isPermissionDenied = (code: string) => code === '42501' || code === 'PGRST301';

  const getDbErrorMsg = (e: any): string => {
    if (!e) return 'Unknown error';
    if (isTableMissing(e.code) || e.message?.includes('does not exist')) {
      return 'Database tables not set up yet. Click the ⚙️ DB Setup tab and run the SQL in Supabase first.';
    }
    if (isPermissionDenied(e.code) || e.message?.includes('permission denied') || e.message?.includes('row-level security')) {
      return 'Permission denied. Make sure you ran the DB Setup SQL. Your account must have admin or super_admin role in the profiles table.';
    }
    if (e.message?.includes('placeholder') || e.message?.includes('Failed to fetch') || e.message?.includes('NetworkError')) {
      return 'Cannot connect to Supabase. Check that VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY are set correctly in your environment variables.';
    }
  ...
  const fetchAnnouncements = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .limit(10);
      if (error) {
        setDbError(getDbErrorMsg(error));
      } else {
        setDbError(null);
        if (data) setAnnouncements(data);
      }
    } catch(e: any) {
      setDbError(getDbErrorMsg(e));
    ...
    try {
      ...
      if (error) {
        if (!isTableMissing(error.code)) {
          toast({ variant: 'destructive', title: 'Error loading sessions', description: getDbErrorMsg(error) });
        }
      } else {
        if (data) setSessions(data);
      }
    } catch(e: any) {
      toast({ variant: 'destructive', title: 'Error', description: getDbErrorMsg(e) });
  ...
  const handlePostAnnouncement = async () => {
    ...
    try {
      const { error } = await supabase
        .from('announcements')
        .insert([{ title: annTitle.trim(), body: annBody.trim(), tag: annTag, created_at: new Date().toISOString() }]);
      if (error) throw error;
      toast({ title: '✅ Announcement posted!' });
      setAnnTitle(''); setAnnBody(''); setAnnTag('INFO');
      fetchAnnouncements();
    } catch(e: any) {
      toast({ variant: 'destructive', title: 'Failed to post', description: getDbErrorMsg(e) });
  ...
  const handleDeleteAnnouncement = async (id: string) => {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: 'Delete failed', description: getDbErrorMsg(e) });
    }
  };
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert([{
          ...
          time: gsTime,
          venue: gsVenue.trim(),
          game_type: gsType.trim() || null,
          slots: gsSlots ? parseInt(gsSlots) : null,
        }])
        .select()
        .single();
      if (error) throw error;
      ...
      toast({ variant: 'destructive', title: 'Failed to add session', description: getDbErrorMsg(e) });
  ...
  const handleAddSchedule = async () => {
    ...
    try {
      const { error } = await supabase.from('schedules').insert([{
        ...
        date: schDate,
        time: schTime,
        location: schLocation.trim() || null,
      ...
      toast({ variant: 'destructive', title: 'Failed to add schedule', description: getDbErrorMsg(e) });
  ...
  return (
    <div className={`${adminBg} border-[3px] border-[#0A0A0A] neubrutalism-box p-6`}>
      ...
      {/* DB Error Banner */}
      {dbError && (
        <div className="bg-[#0A0A0A] border-[3px] border-[#FF3B3B] p-4 flex items-start gap-3 mb-4">
          <span className="text-[#FF3B3B] text-xl shrink-0">⚠️</span>
          <div className="flex-1">
            <p className="font-bold text-white text-sm">{dbError}</p>
            {role === 'super_admin' && (
              <button
                onClick={() => setActiveTab('setup')}
                className="mt-2 bg-[#FFE500] text-[#0A0A0A] font-bold uppercase text-xs px-3 py-1.5 border-[2px] border-[#0A0A0A] hover:bg-white"
              >
                ⚙️ Open DB Setup
              </button>
            )}
        ...
        <div className="space-y-4">
          <div className="bg-[#0A0A0A] text-white border-[3px] border-[#0A0A0A] p-5">
            ...
            <ol className="space-y-2 text-sm font-bold list-decimal list-inside text-gray-300">
              <li>Open your <a href="https://supabase.com/dashboard" target="_blank" rel="noreferrer" className="text-[#FFE500] underline hover:text-white">Supabase Dashboard</a></li>
              <li>Select your project → <span className="text-white">SQL Editor</span></li>
          ...
          <div className="bg-white border-[3px] border-[#0A0A0A] p-5 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500 font-bold mt-0.5">Sign in, find your project, open SQL Editor.</p>
            </div>
            <a
